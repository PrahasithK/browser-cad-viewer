...
async function main() {
  ...
  // Face-pick: click dead center of the isolated body.
  ...
  await page.waitForTimeout(300);
  assert(true, 'Face picked, Sketch entered drawing phase');

  // Draw a rectangle profile entirely on one side of the U axis (which passes through the
  // click's own screen position), large enough that its revolved tool solid, once mirrored
  // inward, substantially overlaps the target body's real material.
  ...
  assert(await kindToggle.isVisible(), 'Finish-mode toggle appears once profile is complete');

  await panel.locator('.kind-btn', { hasText: 'Revolve' }).click();
  await page.waitForTimeout(200);
  await panel.locator('input[type=number]').first().fill('90');
  await page.waitForTimeout(150);
  ...
  const revolveBtn = panel.locator('.extrude-btn', { hasText: 'Revolve' });
  assert(await revolveBtn.isVisible(), 'Revolve apply button visible');
  ...
  await page.waitForFunction(() => !document.querySelector('.sketch-panel'), { timeout: 60000 }).catch(async () => {
    ...
    if (err) throw new Error('REVOLVE_CUT_ERROR: ' + err);
  });
  await page.waitForTimeout(500);
  ...
  assert(delta < 0, `Volume DECREASED (delta=${delta}) — confirms a real Cut happened, not a no-op or a fuse (this feature's own known failure mode before its mirror-plane fix: BRepAlgoAPI_Cut reporting IsDone()==true while removing nothing, because the revolved tool solid sat almost entirely outside the target body)`);
