...
const PICKED_FACE_COLOR = 0xffee00; // matches ShellRendererService/FilletChamferRendererService's own "you picked this" yellow
const HOVER_FACE_COLOR = 0x2a6fdb; // matches the app-wide selection highlight tint
const FACE_OFFSET = 0.05; // same z-fighting guard SketchRendererService/ShellRendererService's own highlightFace uses

/**
 * Renders the "DraftOverlay" scene group: one translucent highlight mesh per picked face-to-
 * draft, plus a hover-preview highlight for whatever face is under the cursor before it's
 * picked. Duplicates ShellRendererService's exact shape (same multi-face highlight-mesh
 * technique) rather than sharing it directly — Draft and Shell are independent tools with their
 * own scene group identity, the same way FilletChamferRendererService/ShellRendererService stay
 * separate despite being nearly identical in shape (each panel/tool owns its own overlay group).
 */
@Injectable({ providedIn: 'root' })
export class DraftRendererService {
