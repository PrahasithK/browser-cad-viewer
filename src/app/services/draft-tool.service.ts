...
import { DraftRendererService } from './draft-renderer.service';
import { generateId } from '../utils/id-generator.util';
import { toCadBodyEdges } from '../utils/edge-geometry.util';
import { CadBody } from '../models/cad-body.model';
import { IDLE_DRAFT_TOOL, DraftToolState } from '../models/draft-tool.model';
import { DraftRequest, StepWorkerRequest, StepWorkerResponse, WorkerTessellatedBody } from '../workers/step-worker-messages.model';

/**
 * Click-to-select-faces tool for Draft (mold-pull angle), the direct analogue of
 * ShellToolService: activate → pick one or more faces on a single existing body (the faces to
 * apply the draft angle to) → configure the angle → commit. Same one-shot worker-per-call shape
 * ShellToolService/FilletChamferToolService already established (no persistent
 * ModelingSessionService session needed).
 */
@Injectable({ providedIn: 'root' })
export class DraftToolService {
  readonly state = signal<DraftToolState>(IDLE_DRAFT_TOOL);
  ...
  constructor(
    ...
    private readonly renderer: DraftRendererService
  ) {}

  setAngle(angleDeg: number): void {
    if (!Number.isFinite(angleDeg) || angleDeg <= 0 || angleDeg >= 90) return;
    this.state.update((st) => ({ ...st, angleDeg }));
  }

  /** Toggles one face in/out of the pick set — same convention ShellToolService.pickFace/FilletChamferToolService.pickEdge already established. All picks must be on the same body; picking a face on a different body restarts the pick set. */
  ...
  canCommit(): boolean {
    return this.state().picks.length > 0 && this.state().angleDeg > 0 && this.state().angleDeg < 90;
  }

  /** Re-reads the picked body's original STEP source bytes — identical resolution to ShellToolService.resolveTargetBody/FilletChamferToolService.resolveTargetBody. */
  ...
  async commit(): Promise<void> {
    const st = this.state();
    if (!this.canCommit()) throw new Error('Pick at least one face to draft first');
    ...
    try {
      ...
      if (!targetBody || !nodeId) {
        throw new Error(
          "This body has no retained STEP source to re-read (only STEP-imported bodies currently support Draft — sketch/primitive-created bodies aren't supported yet)."
        );
      }

      const result = await this.runWorker({
        requestId: generateId('draft'),
        targetBody,
      ...
      this.state.set(IDLE_DRAFT_TOOL);
      this.renderer.clear();
      this.camera.fitAll();
    } catch (err) {
  ...
  /** One-shot worker call, mirroring ShellToolService.runWorker's shape exactly. */
  private runWorker(req: DraftRequest): Promise<WorkerTessellatedBody> {
    return new Promise<WorkerTessellatedBody>((resolve, reject) => {
      ...
      worker.onmessage = (event: MessageEvent<StepWorkerResponse>) => {
        ...
        if (msg.type !== 'draft.result' || msg.requestId !== req.requestId) return;
        worker.terminate();
        if (!msg.success || !msg.body) {
          reject(new Error(msg.error ?? 'Draft failed'));
          return;
        }
        resolve(msg.body);
      };
      ...
      const request: StepWorkerRequest = { type: 'feature.draft', ...req };
      worker.postMessage(request, [req.targetBody.bytes.buffer]);
    });
  }

  /** Builds the resulting mesh/CadBody and swaps it into the same tree node — mirrors ShellToolService.replaceBodyInScene exactly. */
